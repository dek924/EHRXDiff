...
from cheff.ldm.modules.distributions.distributions import normal_kl, DiagonalGaussianDistribution
from cheff.ldm.models.autoencoder import VQModelInterface, IdentityFirstStage, AutoencoderKL
from cheff.ldm.modules.diffusionmodules.util import make_beta_schedule, extract_into_tensor, noise_like
...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key == "prev_img":
                    xc = super().get_input(batch, cond_key).to(self.device)
                elif cond_key == "table":
                    xc = {}
                    xc["c_crossattn"] = {
                        "table": super().get_input(batch, "table").to(self.device),
                        "attn_mask": super().get_input(batch, "attn_mask").to(self.device),
                    }
                elif cond_key == "table, prev_img":
                    if self.model.conditioning_key == "hybrid":
                        # Hard coding
                        xc = {}
                        xc["c_concat"] = super().get_input(batch, "prev_img").to(self.device)
                        xc["c_crossattn"] = {
                            "table": super().get_input(batch, "table").to(self.device),
                            "attn_mask": super().get_input(batch, "attn_mask").to(self.device),
                            "prev_img": super().get_input(batch, "prev_img").to(self.device)
                        }
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict):
                    if self.model.conditioning_key == "hybrid":
                        c = {}
                        c["c_concat"] = [self.encode_first_stage((xc["c_concat"])).mode().detach()]
                        c["c_crossattn"] = {k: v.to(self.device) for k, v in xc["c_crossattn"].items()}
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                if isinstance(c, dict):
                    if (self.cond_stage_key in ["table, prev_img"]) & (self.model.conditioning_key == "hybrid"):
                        c["c_concat"] = [self.encode_first_stage((c["c_concat"].to(self.device))).mode().detach()]
                        c["c_crossattn"] = {k:v.to(self.device) for k, v in c["c_crossattn"].items()}
                        c["c_crossattn"] = [self.get_learned_conditioning(c["c_crossattn"])]
                    elif (self.model.conditioning_key == "crossattn"):
                        # To use adaptor (key == "table", "image")
                        c["c_crossattn"] = {k:v.to(self.device) for k, v in c["c_crossattn"].items()}
                        c["c_crossattn"] = [self.get_learned_conditioning(c["c_crossattn"])]
                    else:
                        raise NotImplementedError
    ...
    def p_sample_loop(
        ...
        if start_T is not None:
            timesteps = min(timesteps, start_T)
        iterator = (
            tqdm(reversed(range(0, timesteps)), desc="Sampling t", total=timesteps) if verbose else reversed(range(0, timesteps))
        )
        ...
        **kwargs,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log["conditioning"] = xc
            elif self.cond_stage_key == "prev_img":
                xc = {}
                xc["c_concat"] = super().get_input(batch, "prev_img").to(self.device)
            elif self.cond_stage_key == "table":
                xc = {}
                xc["c_crossattn"] = {
                    "table": super().get_input(batch, "table").to(self.device),
                    "attn_mask": super().get_input(batch, "attn_mask").to(self.device),
                }
            elif self.cond_stage_key == "table, prev_img":
                if self.model.conditioning_key == "hybrid":
                    # Hard coding
                    xc = {}
                    xc["c_concat"] = [super().get_input(batch, "prev_img").to(self.device)]
