...
class CLIPWrapper(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = CosineAnnealingWarmupRestarts(
            optimizer, first_cycle_steps=500, cycle_mult=1.0, max_lr=lr, min_lr=0, warmup_steps=100
        )
...
class CustomCLIPWrapper(CLIPWrapper):
    def __init__(self, image_encoder, text_encoder, minibatch_size, learning_rate=3e-3, kl_coeff=1.0, avg_word_embs=False):
    ...
    def training_step(self, train_batch, idx):
        ...
        with torch.no_grad():
            ...
            loss += (
                (
                    F.kl_div(F.log_softmax(image_logits_notemp * self.sink_temp, dim=-1), img_target, reduction="batchmean")
                    + F.kl_div(F.log_softmax(image_logits_notemp.t() * self.sink_temp, dim=-1), txt_target, reduction="batchmean")
                )
                / 2
                * self.kl_coeff
            )
    ...
    def encode_text(self, inputs, teacher=False):
        if self.avg_word_embs:
            ...
            embeddings = torch.sum(sequence_output * inputs["attention_mask"].unsqueeze(-1), dim=1) / torch.clamp(
    ...
    def forward(self, images, text):
        logits = (
            F.normalize(self.model.encode_image(images), dim=1)
            @ F.normalize(self.encode_text(text), dim=1).t()
    ...
    def configure_optimizers(self):
        ...
        # Use pip install 'git+https://github.com/katsura-jp/pytorch-cosine-annealing-with-warmup'
        lr_scheduler = CosineAnnealingWarmupRestarts(
            optimizer, first_cycle_steps=self.num_training_steps, cycle_mult=1.0, max_lr=lr, min_lr=0, warmup_steps=2000
        )

        return {"optimizer": optimizer, "lr_scheduler": lr_scheduler}


class CustomTabViTCLIPWrapper(CLIPWrapper):
    def __init__(
        self,
        tab_encoder,
        batch_size,
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "./configs/ViT.yaml")) as fin:
            config = yaml.safe_load(fin)["ViT-B/32_256"]
        super().__init__("ViT-B/32", config, batch_size)
        del self.model.transformer
        import clip
        image_encoder, _ = clip.load("ViT-B/32", jit=False)
        self.model.visual.proj = None
        self.model.transformer = tab_encoder

        checkpoint = image_encoder.visual.state_dict()
        checkpoint = {k:v for k, v in checkpoint.items() if "positional_embedding" not in k}
        missing, unexpected = self.model.visual.load_state_dict(checkpoint, strict=False)
    ...
    def training_step(self, train_batch, batch_idx):
        image, table, attnmask = train_batch["img"], train_batch["tab"], train_batch["attn_mask"]

        # calculate original statistics
        ims = F.normalize(self.model.encode_image(image), dim=1)
        tabs = F.normalize(self.encode_tab(table, attnmask), dim=1)

        image_logits_notemp = ims @ tabs.t()
        image_logits = image_logits_notemp * self.model.logit_scale.exp()
        ground_truth = torch.arange(len(image_logits)).long().to(image_logits.device)
        loss = (F.cross_entropy(image_logits, ground_truth) + F.cross_entropy(image_logits.t(), ground_truth)) / 2
        acc_i = (torch.argmax(image_logits, 1) == ground_truth).float().mean()
        acc_t = (torch.argmax(image_logits.t(), 1) == ground_truth).float().mean()

        self.log_dict({"loss": loss, "acc": (acc_i + acc_t) / 2}, prog_bar=True)
        self.model.logit_scale.data = self.model.logit_scale.data.clamp(-np.log(100), np.log(100))
        return loss

    def encode_tab(self, emb, attn_mask, teacher=False):
        return self.teacher.transformer(emb, attn_mask) if teacher else self.model.transformer(emb, attn_mask)
    ...
    def forward(self, images, tab):
        logits = (
            F.normalize(self.model.encode_image(images), dim=1)
            @ F.normalize(self.encode_tab(tab), dim=1).t()
            * self.model.logit_scale.exp()
        )
        return logits, logits.t()
