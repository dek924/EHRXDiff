...
def get_parser():
    ...
    # misc
    parser.add_argument(
        "--max_event_size", type=int, default=1024, help="max event size to crop to"
    )
    parser.add_argument(
        "--min_event_size",
        type=int,
    ...
    parser.add_argument(
        "--obs_size", type=int, default=48, help="observation window size by the hour"
    )
    parser.add_argument(
        "--gap_size", type=int, default=0, help="time gap window size by the hour"
    )
    parser.add_argument(
        "--pred_size", type=int, default=0, help="prediction window size by the hour"
    )
    parser.add_argument(
        "--long_term_pred_size",
        type=int,
    ...
    parser.add_argument(
        "--prescription",
        action="store_true",
        help="whether to include prescription task or not",
    )
    ...
    ### For image ###
    parser.add_argument(
        "--debugging", action="store_true", help="debugging mode or not"
    )
    parser.add_argument("--use_image", action="store_true", help="use cxr image or not")
    parser.add_argument(
        "--img_datadir",
        metavar="IMGDIR",
        default=None,
        help="directory containing image files of the given ehr (--ehr)."
    )
...
def main(args):
    ...
    # Copy dataset_split.json to the save directory (to simplify the code) 
    import shutil
    for file_name in ["dataset_split", "dataset_split_subject"]:
        split_json_dest = os.path.join(args.dest, f"{file_name}.json")
        shutil.copy(os.path.join(os.path.dirname(os.path.abspath(__file__)), f"meta_info/{file_name}.json"), split_json_dest)
