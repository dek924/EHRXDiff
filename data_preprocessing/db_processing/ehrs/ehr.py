...
class EHR(object):
    def __init__(self, cfg):
        ...
        self.img_data_dir = cfg.img_datadir
        ...
        self.prescription = cfg.prescription

        # time filter options (for image)
        ...
        self.use_admission_table = cfg.use_admission_table
    ...
    def build_cohorts(self, icustays, cached=False):
        ...
        if not self.use_admission_table:
    ...
    def process_tables(self, cohorts, spark):
        ...
        for table in self.tables:
            ...
            if "filter_key" in table:
                events = events.filter(F.col(table["filter_key"]).isin(table["filter_value"]))
            ...
            if infer_icustay_from_hadm_key:
                ...
                if table["timeoffsetunit"] =='abs':
                    events = (
                        events.withColumn(
                            "TEMP_TIME",
                            (F.col(timestamp_key).cast("long") - F.col("INTIME").cast("long")) / 60
            ...
            if table["timeoffsetunit"] == 'abs':
                events = events.withColumn("TIME", (F.col(timestamp_key).cast("long") - F.col("INTIME").cast("long")) / 60)
            ...
            if self.rolling_from_last:
                ...
                if not self.use_image:
                    ...
                    # For X-ray, use data within "obs_size" hours from the imaging time
                    events = events.filter(F.col("TIME") >= 0).filter((F.col("OUTTIME") - F.col("TIME")) >= 0).filter((F.col("OUTTIME") - F.col("TIME")) < obs_size * 60)
            events = events.drop("INTIME", "OUTTIME", self.hadm_key, "stay_id")
    ...
    def make_input(self, cohorts, events, spark):
        def _make_input(events):
            ...
            df = events[["TIME", "INPUTS", "TYPES", "DPES", self.icustay_key]].sort_values("TIME")
            df = df.iloc[-self.max_event_size:]

            if self.rolling_from_last or self.first_to_last:
                raise NotImplementedError
            else:
            ...
            with open(os.path.join(self.cache_dir, self.ehr_name, f"{stay_id}.pkl"), "wb") as f:
                pickle.dump(data, f)
            return pd.DataFrame({"TIME": events["TIME"]})
        ...
        _data_key = self.icustay_key 
        print(f"Use data key : {_data_key}")
        schema = "TIME int"  # Adjust schema based on what is returned
        events.select(["TIME", "INPUTS", "TYPES", "DPES", self.icustay_key]) \
            .groupby(_data_key) \
            .applyInPandas(_make_input, schema=schema) \
        ...
        cohorts[['hi_start', 'time']] = None
        if self.use_image:
            active_stay_ids = [str(i.split(".")[0]) for i in os.listdir(os.path.join(self.cache_dir, self.ehr_name))]  
        else:
        ...
        cohorts = cohorts[cohorts[_data_key].isin(active_stay_ids)]
        ...
        cohorts = cohorts[['LOS', 'OUTTIME', 'dicom_id', 'hadm_id', 'split_42', 'subject_id']]
        for stay_id_file in tqdm(os.listdir(os.path.join(self.cache_dir, self.ehr_name))):
            ...
            corresponding_idx = cohorts.index[cohorts[_data_key]==stay_id][0]
            row = cohorts.loc[corresponding_idx]
            for col in cohorts.columns:
                if col in ["INTIME", "hi_start", "time"]:
                    continue
                try:
                    stay_g.attrs[col] = row[col]
                except:
                    try:
                        stay_g.attrs[col] = row[col].astype("str")
                    except:
                        import pdb; pdb.set_trace()
                        
            # If want to acceleate keys split in datasets, read df selectively
            cohorts.at[corresponding_idx, 'hi_start'] = list(data['hi_start'])
