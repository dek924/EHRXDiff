...
class DINOMIMICClassification(nn.Module):
    def __init__(self, args, attr, num_targets=2, img_size=224):
        ...
        self.mlps = nn.ModuleDict(
            {
                k: FeatureClassifier(in_channels=embed_dim, hidden_channels=embed_dim // 2, num_targets=self.num_targets)
                for k in attr
            }
        )
...
class DINOCXRClassification(nn.Module):
    def __init__(self, args, num_targets=2, img_size=224):
        super(DINOCXRClassification, self).__init__()
        # ============ building network ... ============
        # if the network is a Vision Transformer (i.e. vit_tiny, vit_small, vit_base)
        if args.arch in vits.__dict__.keys():
            model = vits.__dict__[args.arch](patch_size=args.patch_size, num_classes=0)
        ...
        # ============ building classification head ... ============
        self.mlps = FeatureClassifier(in_channels=embed_dim, hidden_channels=embed_dim // 2, num_targets=self.num_targets)

    def get_embedding(self, input, n_last_blocks):
        if "vit" in self.args.arch:
            intermediate_output = self.model.get_intermediate_layers(input, n_last_blocks)
            output = torch.cat([x[:, 0] for x in intermediate_output], dim=-1)
            if self.args.avgpool_patchtokens:
    ...
    def forward(self, data, n):
        if self.args.full_finetune:
            output = self.get_embedding(input=data["img"], n_last_blocks=n)
        else:
            with torch.no_grad():
                output = self.get_embedding(input=data["img"], n_last_blocks=n)
...
class DINOMIMICLOCALClassification(nn.Module):
    def __init__(self, args, combs, num_targets=2, img_size=224):
        super(DINOMIMICLOCALClassification, self).__init__()
        # ============ building network ... ============
        # if the network is a Vision Transformer (i.e. vit_tiny, vit_small, vit_base)
        if args.arch in vits.__dict__.keys():
            model = vits.__dict__[args.arch](patch_size=args.patch_size, num_classes=0)
    ...
    def forward(self, data, n, data_key="comb"):
        head_idx = data[data_key]
        if self.args.full_finetune:
            output = self.get_embedding(input=data["img"], n_last_blocks=n)
        else:
            with torch.no_grad():
